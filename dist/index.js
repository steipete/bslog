...
function resolveQueryLimit(limit, defaultLimit) {
...
function setConfig(key, value) {
  ...
  switch (key) {
    ...
    case "limit": {
      ...
      if (limit === undefined) {
        console.error(source_default2.red(`Limit must be an integer between 1 and ${MAX_QUERY_LIMIT}`));
        process.exit(1);
      }
...
var NUMERIC_LITERAL_PATTERN = /^[+-]?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?$/;
...
function parseArgumentValue(key, value) {
  if (key === "limit") {
