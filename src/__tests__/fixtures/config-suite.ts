...
describe("Config Utilities", () => {
  ...
  const protectedDirs = [homeConfigDir, process.env.BSLOG_CONFIG_DIR].filter((dir): dir is string =>
    Boolean(dir),
  );
  ...
  let fileSpies: ReturnType<typeof watchFileOperations>;

  function watchFileOperations() {
    return fileOperations.map((operation) => spyOn(fs, operation));
  }

  beforeAll(() => {
    fileSpies = watchFileOperations();
  });

  afterAll(() => {
    try {
      for (const spy of fileSpies) {
        for (const [path] of spy.mock.calls) {
          const target = resolve(String(path));
          for (const dir of protectedDirs) {
            expect(target === dir || target.startsWith(`${dir}${sep}`)).toBe(false);
          }
        }
      }
    } finally {
  ...
  describe("config path isolation", () => {
    ...
    it.each([undefined, ""])("should use the default path when the override is %p", (override) => {
      ...
      expect(getConfigFile()).toBe(join(homeConfigDir, "config.json"));
    });
  });
